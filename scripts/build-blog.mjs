...
// sharp 為選用相依：沒安裝也能照常產生 HTML，只是不縮圖（維持原本零相依也能跑的精神）。
let sharp = null;
try { sharp = (await import('sharp')).default; }
catch { console.warn('⚠ 未安裝 sharp，略過自動縮圖（如需縮圖請在 neowise-website/ 執行 npm install）。'); }
...
const ASSETS_DIR = join(BLOG_OUT_DIR, 'assets');       // blog/assets/
const SITE_URL = 'https://neowise.com.tw';
const GA_ID = 'G-QVQJDMZYRH';

// 縮圖設定：超過此寬度才等比縮小並覆寫原檔（考慮 2x 高解析螢幕，1600 足夠；舊圖多為 1200px 內不受影響）
...
/* ---------- 自動縮圖 ---------- */
// 掃描 blog/assets 下所有圖檔，寬度超過 MAX_IMG_WIDTH 就等比縮小並「覆寫原檔」。
// 已在範圍內的圖（含現有舊圖）不動，避免重複壓縮失真。
async function optimizeImages() {
  ...
  for (const file of files) {
    try {
      ...
      const rel = file.slice(SITE_ROOT.length + 1).replace(/\\/g, '/');
  ...
  console.log(`  ✔ 縮圖檢查完成（縮小 ${resized} 張，其餘已在尺寸內）`);
}
